...
namespace LMX
{
	...
	{
		setInt(name, TextureUnits[name] = (GLuint)textureCount++);
		return (GLuint)textureCount;
	}
	...
	void Shader::ShaderErrorCheck(GLuint shader, GLenum pname, const char* shaderPath, bool logInfo, GLint* code, char* log)
	{
		...
		if (!success)
		{
			...
			LMX_ASSERT(logInfo, "ShaderError: {1}\n{0}", infoLog, shaderPath);
	...
	GLint Shader::GetUni(std::string name) const
	{
		...
		if (UniLoc == -1)
			LMX_ERROR("Unable to locate uniform named: {0}", name);
		return UniLoc;
	}
