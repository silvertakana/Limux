	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				// profiling output.
				LMX_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				InternalEndSession();
			}
			...
			{
				LMX_ERROR("Instrumentor could not open results file '{0}'.", filepath);
			}
		}
