...
namespace LMX
{
	...
	void Camera::Control(GLFWwindow* window, float speed, float sensiv)
	{
		...
		if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS)
		{
			...
			float rotX = sensiv * (float)(mouseY - (height / 2)) / height;
			float rotY = sensiv * (float)(mouseX - (width / 2)) / width;
